    ...
    <section class="content">
        <div class="box box-info">
            <div class="box-header">
                <h3 class="box-title">Form Edit Permohonan Pemutusan</h3>
            ...
            <div class="box-body">
                ...
                <form class="form-horizontal" action="{{ route('pemutusan.update', $pemutusan->id) }}" method="POST">
                    @method('PUT')
                    @csrf
                            ...
                            <select class="form-control select2 @error('pelanggan_id') is-invalid @enderror"
                                ...
                                @foreach ($pelanggans as $pelanggan)
                                    <option value="{{ $pelanggan->id }}"
                                        {{ old('pelanggan_id', $pemutusan->pelanggan_id) == $pelanggan->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <textarea class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsi" name="deskripsi" rows="3"
                                placeholder="Deskripsikan tujuan pemutusan..." required>{{ old('deskripsi', $pemutusan->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                <input type="text" class="form-control @error('lokasi') is-invalid @enderror"
                                    id="lokasi" name="lokasi" value="{{ old('lokasi', $pemutusan->lokasi) }}"
                    ...
                    <div class="form-group">
                        <label for="jumlah_tunggakan" class="col-sm-2 control-label">Jumlah Tunggakan</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="jumlah_tunggakan" name="jumlah_tunggakan"
                                value="Rp {{ number_format($pemutusan->jumlah_tunggakan, 2, ',', '.') }}" readonly>
                            <small class="text-muted">Diambil otomatis dari tagihan yang belum dibayar.</small>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-10">
                            <select class="form-control @error('status') is-invalid @enderror" id="status" name="status"
                                ...
                                <option value="pending"
                                    {{ old('status', $pemutusan->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="proses"
                                    {{ old('status', $pemutusan->status) == 'proses' ? 'selected' : '' }}>Proses</option>
                                <option value="disetujui"
                                    {{ old('status', $pemutusan->status) == 'disetujui' ? 'selected' : '' }}>Disetujui
                                </option>
                                <option value="ditolak"
                                    {{ old('status', $pemutusan->status) == 'ditolak' ? 'selected' : '' }}>Ditolak</option>
                            </select>
                            @error('status')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="form-group" id="alasan-ditolak-section"
                        style="display: {{ old('status', $pemutusan->status) == 'ditolak' ? 'block' : 'none' }};">
                        <label for="alasan_ditolak" class="col-sm-2 control-label">Alasan Ditolak <span
                                class="text-red">*</span></label>
                        <div class="col-sm-10">
                            <textarea class="form-control @error('alasan_ditolak') is-invalid @enderror" id="alasan_ditolak" name="alasan_ditolak"
                                rows="3" placeholder="Alasan penolakan permohonan...">{{ old('alasan_ditolak', $pemutusan->alasan_ditolak) }}</textarea>
